...
def run_analysis(train_csv, test_csv, output_csv):
    ...
    exclude = ['specimen_id','subject_id', 'actual_day_relative_to_boost', 'planned_day_relative_to_boost', 'specimen_type', 'visit', 'ethnicity', 'year_of_birth', 'date_of_boost', 'dataset', 'timepoint']
    ...
    y_train = day_14_data_train['ENSG00000277632.1']
    ...
    # Ensure all training columns are in test data
    ...
    day_0_values = day_0_data_test_unique['ENSG00000277632.1']
    fold_change = results_df['CCL4'] / day_0_values.reindex(results_df['subject_id']).values
    results_df['FC_CCL4'] = fold_change
    results_df.replace([np.inf, -np.inf], np.nan, inplace=True)
    results_df.dropna(inplace=True)
    results_df['CCL4_rank'] = results_df['CCL4'].rank(ascending=False, method='min').astype(int)
    results_df['CCL4-FC_rank'] = results_df['FC_CCL4'].rank(ascending=False, method='min').astype(int)
    results_df.drop(columns=['CCL4', 'FC_CCL4'], inplace=True)
    results_df.to_csv(f'{output_csv}/Gene-Expression_testing.csv', index=False)
